...
export default function ABTestPage() {
  ...
  const buildVideos = (v: Variant): MockupVideo[] => [
    ...
    {
      thumbnailUrl: competitorThumb,
      title: compTitle,
      channelName: compChannel,
      channelVerified: true,
      viewCount: compViews,
      uploadDate: compDate,
      ...
      <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="mx-auto max-w-2xl px-4 sm:px-6 py-20 text-center">
          ...
          <p className="mt-4 text-neutral-400 text-lg">
            Upload 2-3 thumbnail variants and compare them side-by-side in a real YouTube search mockup with CTR scores.
          </p>
          <Link href="/redeem" className="inline-block mt-8">
            <Button size="lg" className="bg-violet-600 hover:bg-violet-500 text-white glow-purple">
              Upgrade to Pro — $15/month
            </Button>
          </Link>
          <Link href="/tool" className="block mt-4">
            <Button variant="link" className="text-neutral-400">Use the free tool instead →</Button>
          </Link>
        </div>
      </div>
    );
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-neutral-400">
            Compare up to 3 thumbnail variants side-by-side in a real YouTube search mockup. CTR scores update automatically.
          </p>
        </div>
        ...
        {bestVariant && (
          <div className="mb-6 flex items-center gap-3 rounded-lg border border-green-500/20 bg-green-500/5 px-4 py-3 text-sm">
            ...
            <span className="text-neutral-200">
              <span className="text-white font-medium">Variant {bestVariant.id.toUpperCase()}</span> is winning with a CTR score of{' '}
              <span className="text-green-400 font-medium">{bestVariant.ctr!.score}/100</span>
            </span>
          </div>
        )}
        ...
        <div className="mb-8 rounded-2xl border border-white/10 bg-[#111] p-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              ...
              <Input value={keyword} onChange={(e) => setKeyword(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
            </div>
            <div>
              <Label className="text-neutral-400 text-xs">Channel name</Label>
              <Input value={channelName} onChange={(e) => setChannelName(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
            </div>
            <div>
              <Label className="text-neutral-400 text-xs">View count</Label>
              <Input value={viewCount} onChange={(e) => setViewCount(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
            </div>
            <div>
              <Label className="text-neutral-400 text-xs">Upload date</Label>
              <Input value={uploadDate} onChange={(e) => setUploadDate(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
            </div>
            <div>
              <Label className="text-neutral-400 text-xs">Duration</Label>
              <Input value={duration} onChange={(e) => setDuration(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
            </div>
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-3 content-start">
              <div className="col-span-2">
                ...
                <Input value={compTitle} onChange={(e) => setCompTitle(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
              </div>
              <div>
                <Label className="text-neutral-400 text-xs">Channel</Label>
                <Input value={compChannel} onChange={(e) => setCompChannel(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
              </div>
              <div>
                <Label className="text-neutral-400 text-xs">Views</Label>
                <Input value={compViews} onChange={(e) => setCompViews(e.target.value)} className="mt-1 bg-[#0a0a0a] border-white/10 text-white" />
              </div>
            </div>
          </div>
        </div>
          ...
          {variants.map((v, idx) => (
            <div key={v.id} className="rounded-2xl border border-white/10 bg-[#111] p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                {variants.length > 2 && (
                  <button onClick={() => removeVariant(v.id)} className="text-neutral-500 hover:text-red-400 transition-colors" aria-label="Remove variant">
              ...
              <div className="mt-4">
                {v.scoring ? (
                  <div className="flex items-center gap-2 text-sm text-neutral-400">
                    <Loader2 className="h-4 w-4 animate-spin text-violet-400" /> Scoring…
                  </div>
                ) : v.ctr ? (
                  <div className="flex items-center gap-3">
                    <div className="flex items-end gap-1">
                      <span className={`text-3xl font-bold ${
                        v.ctr.score >= 80 ? 'text-green-400' :
                        v.ctr.score >= 65 ? 'text-violet-400' :
                        v.ctr.score >= 45 ? 'text-yellow-400' : 'text-red-400'
                      }`}>{v.ctr.score}</span>
              ...
              {v.thumb && (
                <div className="mt-4">
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="mt-3 w-full border-white/10 text-white hover:bg-white/5"
                  >
        ...
        {variants.length < 3 && (
          <button
            onClick={addVariant}
            className="mt-6 w-full rounded-2xl border-2 border-dashed border-white/10 py-6 text-neutral-400 hover:border-violet-500/40 hover:text-violet-400 transition-colors flex items-center justify-center gap-2"
          >
