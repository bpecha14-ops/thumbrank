  ...
  const cards = [
    {
      ...
      desc: 'Upload your thumbnail and see it in a real YouTube search mockup.',
      href: '/tool',
      cta: 'Open tool',
      pro: false,
    },
    {
      ...
      title: 'A/B Testing',
      desc: 'Compare 2-3 thumbnail variants side-by-side with CTR scores.',
      href: '/ab-test',
      cta: 'Start A/B test',
      pro: true,
    },
    {
      icon: Gauge,
      title: 'CTR Score',
      desc: 'Get an instant 0-100 score based on contrast, color, and readability.',
      href: '/tool',
      cta: 'Check a score',
      pro: true,
    },
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${isPro ? 'bg-violet-600 text-white' : 'bg-white/10 text-neutral-300'}`}>
                  {isPro ? 'PRO' : 'FREE'}
                </span>
              ...
              {isPro ? (
                ...
                <p className="mt-2 text-lg text-white">
                  {profile?.remaining !== null && profile?.remaining !== undefined
                    ? `${profile.remaining} of 3 free previews left this month`
                    : '3 free previews per month'}
                </p>
              )}
            </div>
            {!isPro && (
              <Link href="/redeem">
                <Button className="bg-violet-600 hover:bg-violet-500 text-white shrink-0">
                  <KeyRound className="mr-2 h-4 w-4" /> Upgrade to Pro
                </Button>
              </Link>
            )}
          </div>
              ...
              <div key={card.title} className="group rounded-2xl border border-white/10 bg-[#111] p-6 hover:border-violet-500/30 transition-colors">
                ...
                <Link href={card.href} className="block mt-4">
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full border-white/10 text-white hover:bg-white/5 group-hover:border-violet-500/30"
                  >
                    {card.cta} <ArrowRight className="ml-2 h-3 w-3" />
                  </Button>
                </Link>
              </div>
            );
        ...
        {!isPro && (
          <div className="mt-8 rounded-2xl border border-violet-500/20 bg-gradient-to-b from-violet-600/10 to-[#111] p-8 text-center">
            ...
            <p className="mt-2 text-sm text-neutral-400 max-w-md mx-auto">
              Unlimited previews, A/B testing with 2-3 variants, and instant CTR scores — all for $15/month.
            </p>
            <Link href="/redeem" className="inline-block mt-6">
              <Button className="bg-violet-600 hover:bg-violet-500 text-white glow-purple">
